...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="显示启动横幅（默认开启，可用 --no-banner 关闭）",
    )
    parser.add_argument(
        "--web-enrich",
        action=argparse.BooleanOptionalAction,
...
def main() -> None:
    ...
    try:
        all_results = run_probe(args)
    except ValueError as exc:
        parser.error(str(exc))
        return

    if args.only_alive:
        results = [item for item in all_results if item.alive]
    else:
        results = all_results

    print_results(results)
    print_summary(all_results, shown_total=len(results))
