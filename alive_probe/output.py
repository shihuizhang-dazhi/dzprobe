...
def print_results(results: Iterable[ProbeResult]) -> None:
    ...
    show_status = any(row.status > 0 for row in rows)
    ...
    for result in rows:
        ...
        if show_status:
            row_parts.append(colorize_cell(str(result.status) if result.status else "-", 5, style_for_status(result.status)))
        if show_server:
            row_parts.append(
                colorize_cell(
        ...
        title_code = Style.WHITE if result.title else (Style.RED if result.error else Style.DIM)
